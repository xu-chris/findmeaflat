...
React Email versions may automate some structural attributes, but FindMeAFlat's
current templates use its Elixir mail seam. Do not add or upgrade React Email
for this checklist. Regardless of renderer, verify heading hierarchy,
descriptive alt and link text, contrast, linked images, `lang`/`dir`, and title
behavior in actual output.
